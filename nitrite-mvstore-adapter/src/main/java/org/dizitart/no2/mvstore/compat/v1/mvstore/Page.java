...
import static org.dizitart.no2.mvstore.compat.v1.mvstore.DataUtils.PAGE_TYPE_LEAF;
...
public abstract class Page implements Cloneable {
     ...
     */
    private static final int PAGE_MEMORY = MEMORY_OBJECT +           // this
     ...
     */
    static final int PAGE_NODE_MEMORY = PAGE_MEMORY +             // super
     ...
     */
    static final int PAGE_LEAF_MEMORY = PAGE_MEMORY +             // super
    ...
    static Page createEmptyNode(MVMap<?, ?> map) {
        return createNode(map, EMPTY_OBJECT_ARRAY, SINGLE_EMPTY, 0, PAGE_NODE_MEMORY + MEMORY_POINTER + PAGE_MEMORY_CHILD); // there is always one child
    }
    ...
    private void read(ByteBuffer buff, int chunkId) {
        ...
        if (isLeaf() != ((type & 1) == PAGE_TYPE_LEAF)) {
            throw DataUtils.newIllegalStateException(DataUtils.ERROR_FILE_CORRUPT, "File corrupted in chunk {0}, expected node type {1}, got {2}", chunkId, isLeaf() ? "0" : "1", type);
        }
        if (!isLeaf()) {
            readPayLoad(buff);
        }
...
//                    "Memory calculation error " + memory + " != " + calculateMemory();
    ...
    public static final class PageReference {
        ...
        @Override
        public String toString() {
            return "Cnt:" + count + ", pos:" + (pos == 0 ? "0" : DataUtils.getPageChunkId(pos) + "-" + DataUtils.getPageOffset(pos) + ":" + DataUtils.getPageMaxLength(pos)) + ((page == null ? DataUtils.getPageType(pos) == 0 : page.isLeaf()) ? " leaf" : " node") + ", page:{" + page + "}";
        }
    }
    ...
    private static class NonLeaf extends Page {
        ...
        @Override
        protected int calculateMemory() {
            return super.calculateMemory() + PAGE_NODE_MEMORY + getRawChildPageCount() * (MEMORY_POINTER + PAGE_MEMORY_CHILD);
        }
