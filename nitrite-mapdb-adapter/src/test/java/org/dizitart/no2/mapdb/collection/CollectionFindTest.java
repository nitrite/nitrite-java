...
import static org.dizitart.no2.mapdb.TestUtil.isSorted;
...
public class CollectionFindTest extends BaseCollectionTest {
    ...
    public void testFindWithIterableEqual() {
        insert();
        DocumentCursor ids = collection.find(where("list").eq(Lists.newArrayList("three", "four", "three")));
        assertNotNull(ids);
        assertEquals(ids.size(), 1);
    }
    ...
    public void testFindWithIterableEqualFailForWrongCardinality() {
        insert();
        DocumentCursor ids = collection.find(where("list").eq(Lists.newArrayList("four", "three", "three")));
        assertNotNull(ids);
        assertEquals(ids.size(), 0);
    }
