...
public class JoinedDocumentStreamTest {
    ...
    public void testConstructor() {
        ...
        RecordStream<Pair<NitriteId, Document>> recordStream1 = (RecordStream<Pair<NitriteId, Document>>) mock(
                RecordStream.class);
        DocumentStream documentStream = new DocumentStream(recordStream1, new ProcessorChain());
        Lookup lookup = new Lookup();
        new JoinedDocumentStream(recordStream, documentStream, lookup, new ProcessorChain());
        assertNull(documentStream.getFindPlan());
        assertNull(lookup.getForeignField());
        assertNull(lookup.getTargetField());
        assertNull(lookup.getLocalField());
    }
}
