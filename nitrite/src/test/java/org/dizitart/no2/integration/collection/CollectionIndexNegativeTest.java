...
import static org.dizitart.no2.index.IndexType.FULL_TEXT;
...
public class CollectionIndexNegativeTest extends BaseCollectionTest {
    ...
    public void testCreateFullTextOnNonTextField() {
        insert();
        collection.createIndex(IndexOptions.indexOptions(FULL_TEXT), "birthDay");
    ...
    @Test(expected = IndexingException.class)
    public void testMultipleTextIndex() {
        collection.createIndex(IndexOptions.indexOptions(FULL_TEXT), "body", "lastName");
    }

    @Test(expected = ValidationException.class)
    public void testIndexOnNullField() {
        collection.createIndex(IndexOptions.indexOptions(FULL_TEXT));
    }
}
